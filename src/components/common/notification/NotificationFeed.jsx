...
export const NotificationFeed = (props) => {
    ...
    return (
        <div
            ...
            <img className="w-12 h-12 rounded-full mr-4" src={ImageUtils.createImageSrcFromBufferWithDefaultIsAvatar(notification?.Actor?.avatar?.data)} alt="channel" />
            <div className="flex-1">
                <div className="text-sm">
                    {
                        notification?.type !== 'Subcribe' && <div>
                        <span
                            className="font-semibold">{notification?.Actor?.name}</span> {' ' + StringUtils.getNotifyLabel(notification?.type)}
